...
locals {
  prefix = "vcore-evidence"
  tags = {
    Project    = "V-Core", Environment = "aws-evidence", ManagedBy = "Terraform",
    Repository = "vietdungnguyen2005/Project_Frontend1", ExpiresAt = var.expires_at
  }
}
...
resource "aws_elasticache_replication_group" "redis" {
  ...
  description                = "V-Core evidence cache"
  engine                     = "redis"
  engine_version             = "7.1"
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "SPRING_DATASOURCE_URL", value = "jdbc:postgresql://${aws_db_instance.postgres.address}:5432/vcore?sslmode=require" },
      { name = "SPRING_DATASOURCE_USERNAME", value = "vcore_admin" },
      { name = "DEPLOYMENT_ENVIRONMENT", value = "production" },
      { name = "DB_MAX_POOL_SIZE", value = "8" },
    ...
    secrets = [
      ...
      { name = "VCORE_BFF_SHARED_SECRET", valueFrom = aws_ssm_parameter.bff_secret.arn }
    ],
...
resource "aws_cloudwatch_metric_alarm" "unhealthy_targets" {
  ...
  alarm_description   = "V-Core has an unhealthy ECS target."
